    ...
    return {
        "bool": {
            "should": [
                ...
                {"term": {f"{field}.keyword": {"value": value, "case_insensitive": True}}},
            ],
...
def _exact_values_filter(field: str, values: str | list[str]) -> dict[str, Any]:
    if isinstance(values, str):
        return _exact_filter(field, values)
    return {
        "bool": {
            "should": [_exact_filter(field, value) for value in values],
            "minimum_should_match": 1,
        }
    }
...
def _build_search_body(search_input: LegalDocumentSearchInput) -> dict[str, Any]:
    ...
    if search_input.ngay_co_hieu_luc:
        filters.append(_date_range_filter("ngay_co_hieu_luc", search_input.ngay_co_hieu_luc))
    ...
    return {
        ...
        "_source": {"excludes": ["toan_van", "cleaned_toan_van", "html_with_reference"]},
        "query": {
            "bool": {
                "must": [
                    {
...
async def _search_legal_documents(
    ...
    try:
        ...
        raise RuntimeError(f"Không thể tìm kiếm văn bản trên Elasticsearch: {exc}") from exc
